...
const destinations = [
    ...
    },
    {
        ...
        seasonal_food: ["공주 알밤 (가을 특산)", "연잎밥", "부여 연꽃나물", "공주 밤막걸리"],
        festivals: ["백제문화제 (9-10월)", "공주 산성 걷기 대회 (10월)", "부여 서동연꽃축제 (7월)"],
        accommodations: ["공주 한옥 스테이", "부여 롯데리조트", "공주 금강뷰 게스트하우스"],
        cafes: ["공산성 근처 카페들", "금강 뷰 카페", "제민천 카페거리"],
...
function renderProvinceRow() {
    ...
    Object.entries(provinceGroups).forEach(([groupName, provinces]) => {
        ...
        <div class="prov-group">
            <span class="prov-group-label">${groupName}</span>
            ${provinces.map(p =>
                `<button class="prov-btn ${p === selectedProvince ? "active" : ""}" data-province="${p}">${p}</button>`
            ).join("")}
        </div>`;
    });
...
function renderCityRow() {
    const container = document.getElementById("city-row");
    ...
    container.innerHTML = `
        ...
        <button class="city-btn ${selectedCity === "전체" ? "active" : ""}" data-city="전체">전체</button>
        ${cities.map(c =>
            `<button class="city-btn ${c === selectedCity ? "active" : ""}" data-city="${c}">${c}</button>`
        ).join("")}
    `;
...
function renderCards() {
    const filtered = destinations.filter(d => {
        ...
        const provinceOk = selectedProvince === "전체" || d.province === selectedProvince;
        const cityOk = selectedCity === "전체" || d.city === selectedCity;
        return monthOk && provinceOk && cityOk;
    });
    ...
    let locationLabel = "";
    if (selectedProvince !== "전체") {
        locationLabel = selectedCity !== "전체"
            ? `<span>${selectedProvince} ${selectedCity}</span> `
            : `<span>${selectedProvince}</span> `;
    }
    summary.innerHTML = `<span>${monthNames[selectedMonth - 1]}</span> ${locationLabel}추천 여행지 <span>${filtered.length}곳</span>`;
